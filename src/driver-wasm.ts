...
class DuckDBConnection implements DatabaseConnection {
  ...
  async executeQuery<O>(compiledQuery: CompiledQuery): Promise<QueryResult<O>> {
    ...
    try {
      ...
      return this.formatToResult(result)
    } finally {
  ...
  async *streamQuery<R>(
    ...
    try {
      for await (const result of iter as any) {
        yield this.formatToResult(result)
      }
    } finally {
      ...
      case arrow.Type.Interval:
        if (Array.isArray(value) && value.length >= 3) {
          ...
          if (arr.length >= 4) {
            ...
            const nanos = nanosHigh * 2 ** 32 + nanosLow
  ...
  private likelyBitByBytes(bytes: number[]): boolean {
    ...
    if (bytes.length === 2) {
      ...
      const fullBinary =
        first.toString(2).padStart(8, '0') + second.toString(2).padStart(8, '0')
